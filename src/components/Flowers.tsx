...
const Flowers: React.FC = () => {
    ...
    const [filteredFlowers, setFilteredFlowers] = useState<any[]>([]);
    ...
    const fetchFlowerPrice = async (id: number) => {
        try {
        ...
        const fetchFlowers = async () => {
            try {
                ...
                const flowersWithPrice = await Promise.all(
                    flowersData.map(async (flower: any) => {
                    ...
                    {filteredFlowers.map((flower: any) => (
                        <Col key={flower.id}>
                            <Card className="h-100">
                                <Card.Img 
                                    variant="top" 
                                    src={flower.image || '/bulbasavr.gif'} 
                                    alt={flower.name} 
                                />
                                <Card.Body>
